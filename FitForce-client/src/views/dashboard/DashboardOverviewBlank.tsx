...
import { Container, Box, CircularProgress, Stack, Typography, Tabs, Tab, Card, CardContent, FormControl, InputLabel, Select, MenuItem } from '@mui/material';
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ xs: 'stretch', md: 'stretch' }} sx={{ minHeight: { md: 620 } }}>
                  ...
                  <Box sx={{ flex: 1, display: 'flex' }}>
                    <Stack spacing={2} sx={{ flex: 1 }}>
                      ...
                      <Card variant="outlined" sx={{ flex: 3, display: 'flex', flexDirection: 'column' }}>
                        <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                          ...
                          <Box sx={{ width: '100%', height: { xs: 380, md: 520 }, mb: 2 }}>
                            {loadingInsights ? (
                              ...
                              <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                  <Pie
                                    data={Object.entries(clientStatusCounts).filter(([k]) => k !== 'all').map(([key, value]) => ({ name: key.replace(/_/g, ' '), value }))}
                                    outerRadius={130}
                                    paddingAngle={1}
                                    dataKey="value"
                                  >
                                    {Object.entries(clientStatusCounts).filter(([k]) => k !== 'all').map((entry, index) => (
                                      <Cell key={entry[0]} fill={["#1976d2", "#2e7d32", "#ed6c02", "#9c27b0", "#0088FE", "#00C49F"][index % 6]} />
                                    ))}
                                  </Pie>
                                  <Tooltip />
                                  <Legend />
                          ...
                          {(() => {
                            ...
                            return (
                              <Box sx={{ maxHeight: { xs: 'none', md: 260 }, overflowY: 'auto', overflowX: 'hidden', pr: 1 }}>
                                <Stack spacing={1}>
                        ...
                        <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                          ...
                          <Box sx={{ flex: 1 }}>
                            {(() => {
                              ...
                              const returning = growthData?.returningClients || 0;
                              const pct = total > 0 ? (returning / total) * 100 : 0;
                              // synthesize a flat series over last 8 dates as placeholder
                              const now = new Date();
                              const series = Array.from({ length: 8 }).map((_, i) => {
                                const d = new Date(now.getTime() - (7 - i) * 24 * 60 * 60 * 1000);
                                const dateKey = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
                                return { date: dateKey, value: Number(pct.toFixed(1)) };
                              });
                              return (
                                <ResponsiveContainer width="100%" height="100%">
                                  <LineChart data={series}>
                                    ...
                                    <Legend />
                                    <Line type="monotone" dataKey="value" stroke="#1976d2" strokeWidth={2} dot={false} name="Retention %" />
                                  </LineChart>
                                </ResponsiveContainer>
                              );
                            })()}
