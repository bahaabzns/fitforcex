...
export default function FeatureSection({ featureKey, screenshot }: FeatureSectionProps) {
    ...
    <section className="h-screen flex items-center bg-[var(--bg-primary)] relative scroll-snap-section scroll-snap-center" dir={dir}>
      ...
      <div className="max-w-[1400px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8 w-full py-6 sm:py-8 md:py-12 lg:py-16 relative z-10">
        ...
        >
          ...
          >
            <h2
              className={`w-full text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent mb-4 sm:mb-6 tracking-tight ${
                dir === 'rtl' ? 'text-right' : 'text-left'
              }`}
            >
              {feature.title}
            </h2>
            <ul className={`w-full space-y-3 sm:space-y-4 mb-6 sm:mb-8 ${dir === 'rtl' ? 'text-right' : 'text-left'}`}>
              {Array.isArray(feature.description) &&
                feature.description.map((point: string, index: number) => (
                  <li key={index} className="flex items-start gap-3">
                    <svg className="w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="none">
                      <path d="M2 10L6 14L14 4" stroke="#0EA5E9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      <path d="M6 10L10 14L18 4" stroke="#06B6D4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    <span
                      className={`text-base sm:text-lg md:text-xl text-[var(--text-secondary)] leading-relaxed flex-1 ${
                        dir === 'rtl' ? 'text-right' : 'text-left'
                      }`}
                    >
                      {point}
            ...
            <div className="flex flex-col gap-3 w-full">
              ...
              <p className={`text-sm text-[var(--text-tertiary)] ${dir === 'rtl' ? 'text-right' : 'text-left'}`}>{t.hero.noCreditCard}</p>
            </div>
          </div>

          {/* Screenshot with glass card effect */}
          <div ref={imageRef} className="relative w-full flex justify-center lg:block">
