    ...
    >
      ...
      <div className="section-container w-full relative z-10">
        <div className="text-center mb-6 md:mb-8">
          ...
          <p className="text-base md:text-lg text-[var(--text-secondary)] mb-2 md:mb-3">{t.faq.subtitle}</p>
          <div className="inline-flex items-center gap-2 text-cyan-400 text-xs md:text-sm">
            ...
            <span>Still have questions? Chat with our team →</span>
          </div>
        </div>
            ...
            {faqs.map((faq, index) => (
              ...
              >
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 md:p-5 transition-colors duration-300 ease-in-out ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}
                  dir={dir}
                >
                  <span
                    className={`font-semibold text-[var(--text-primary)] text-sm md:text-base lg:text-lg flex-1 ${dir === 'rtl' ? 'text-right' : 'text-left'}`}
                  >
                    {faq.question}
                  </span>
                  <svg
                    className={`w-5 h-5 md:w-6 md:h-6 text-cyan-400 transition-all duration-300 ease-in-out flex-shrink-0 ${dir === 'rtl' ? 'ml-3 md:ml-4' : 'mr-3 md:mr-4'} ${
                ...
                >
                  <div className="px-4 md:px-5 pb-4 md:pb-5 border-t border-cyan-500/10">
                    <p
                      className={`text-[var(--text-secondary)] text-sm md:text-base leading-relaxed pt-3 md:pt-4 ${dir === 'rtl' ? 'text-right' : 'text-left'}`}
                      dir={dir}
                    >
                      {faq.answer}
                    </p>
