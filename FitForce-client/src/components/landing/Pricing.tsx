  ...
  const plans = [
    {
      ...
      coreFeatures: t.pricing.oneForce.coreFeatures,
      color: "blue",
      cta: t.pricing.cta,
      href: "/register",
    },
    {
      ...
      everythingIn: t.pricing.teamForce.everythingIn,
      teamFeatures: t.pricing.teamForce.teamFeatures,
    ...
    },
    {
      ...
      everythingIn: t.pricing.enterpriseForce.everythingIn,
      enterpriseFeatures: t.pricing.enterpriseForce.enterpriseFeatures,
  ...
  return (
    ...
    >
      ...
      <div className="section-container py-8 w-full relative z-10">
        <div
          ref={headerRef}
          className="text-center mb-8 animate-on-scroll fade-in-up"
        >
          ...
          <div className="inline-flex items-center justify-center gap-1 sm:gap-2 bg-gray-900/60 backdrop-blur-xl border border-cyan-500/30 rounded-xl p-1 sm:p-1.5 max-w-full">
            {(
              [
                "monthly",
                "quarterly",
              ...
              >
                ...
                {period !== "monthly" && (
                  <span
                    className={`text-[9px] sm:text-xs font-bold block ${
                      dir === "rtl" ? "mr-0" : "ml-0"
                    }`}
                  >
        ...
        <div
          ref={cardsRef}
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 mb-6 px-2 sm:px-0"
        >
          {plans.map((plan, index) => (
            <div
              ...
              className={`relative rounded-2xl overflow-hidden backdrop-blur-xl border transition-all duration-300 flex flex-col ${
                plan.popular
                  ? "ring-2 ring-cyan-500/50 shadow-lg shadow-cyan-500/10 border-cyan-500/50 md:transform md:scale-105"
                  : "shadow-md border-cyan-500/20 hover:border-cyan-500/40"
              }`}
            >
              {/* Highlight Badge */}
              <div
              ...
              <div
                ...
                } p-6 text-white pt-12 relative overflow-hidden`}
              >
                {/* Subtle glow overlay */}
                <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent"></div>

                <h3 className="text-3xl md:text-4xl font-black mb-2 relative z-10 bg-gradient-to-r from-white via-cyan-50 to-white bg-clip-text text-transparent drop-shadow-[0_0_8px_rgba(34,211,238,0.3)]">
                  {plan.name}
                </h3>
                <p className="text-white/90 mb-4 text-sm relative z-10">
                  {plan.description}
                </p>
                  ...
                  <div className="mb-3 relative z-10">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="flex items-center gap-1.5 bg-white/10 rounded-full px-2 py-0.5 border border-white/20">
                        <button
                          onClick={() =>
                            setTeamMembers(Math.max(3, teamMembers - 1))
                          }
                        ...
                        <button
                          onClick={() =>
                            setTeamMembers(Math.min(20, teamMembers + 1))
                          }
                    ...
                    {teamMembers > 3 && (
                      <div className="text-[10px] text-cyan-200/80 font-mono bg-white/5 rounded px-2 py-1 border border-white/10">
                        Base (3) + Extra ({teamMembers - 3}) × 800{" "}
                        {dir === "rtl" ? "ج.م" : "LE"}
                      </div>
                    )}
                  </div>
                ...
                <div className="mb-4 relative z-10">
                  {plan.basePrice || plan.isTeam ? (
                    <>
                      <div className="flex items-baseline gap-2 justify-center md:justify-start">
                        <div className="text-4xl md:text-5xl font-bold">
                          {plan.isTeam
                            ? calculateTeamPrice().toLocaleString()
                            : calculatePrice(plan.basePrice!).toLocaleString()}
                        </div>
                    ...
                    <div className="text-4xl font-bold">
                      {t.pricing.enterpriseForce.price}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-900/40 backdrop-blur-xl border-t border-white/10 p-6 flex-1 flex flex-col">
                {/* Core Features / Everything In Section */}
                {(plan.coreFeatures || plan.everythingIn) && (
                  <div className="mb-4">
                    <h4
                      className={`font-semibold text-xs mb-3 tracking-wide ${
                        plan.everythingIn
                          ? "text-emerald-400 bg-emerald-500/10 border-l-4 border-emerald-500 px-3 py-2"
                          : "text-gray-400"
                      }`}
                    >
                      {plan.coreFeatures || plan.everythingIn}
                ...
                {/* Team Features / Enterprise Features Section */}
                {(plan.teamFeatures || plan.enterpriseFeatures) && (
                  <div className="mb-4">
                    <h4 className="text-gray-400 font-semibold text-xs mb-3 tracking-wide">
                      {plan.teamFeatures || plan.enterpriseFeatures}
                    </h4>
                  </div>
                )}
                ...
                <ul className="space-y-3 mb-6 flex-1">
                  {plan.features.map((feature, index) => {
                    const isUnlimited =
                      ...
                      feature.includes("غير محدود");
                    return (
                      <li
                        key={index}
                        className={`flex items-start gap-3 ${
                          isUnlimited
                            ? "bg-cyan-500/10 border border-cyan-500/30 rounded-lg p-2 -m-1"
                            : ""
                        }`}
                      >
                        <svg
                          className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                            isUnlimited ? "text-cyan-400" : "text-cyan-400"
                          }`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
        ...
        <div className="text-center">
          <p className="text-gray-400 text-sm">
            {t.pricing.allPricesTaxIncluded}
          </p>
        </div>
      </div>
    </section>
