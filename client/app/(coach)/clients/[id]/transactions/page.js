    ...
    return (
        <div className="p-6 flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between">
                <h2 className="text-base font-semibold text-[#1D1D1F]">Transactions</h2>
                <button
                    onClick={openAdd}
                    className="text-sm px-4 py-2 rounded-xl bg-[#007AFF] text-white hover:bg-[#0056CC] transition-colors cursor-pointer font-medium"
                >
                    + Add Transaction
                </button>
            </div>
                    ...
                    {Object.keys(byCurrency).length > 0 && (
                        <div className="flex flex-col gap-0.5 mt-1">
                            {Object.entries(byCurrency).map(([cur, amt]) => (
                                <span key={cur} className="text-xs text-[#86868B]">{amt.toLocaleString()} {cur}</span>
                            ))}
                        </div>
                    )}
                </div>
            ...
            {(displayPeriod || isPreStart || hasNoSubscriptions) && (
                <div className={`card border ${
                    hasNoSubscriptions ? "border-[#D2D2D7] bg-[#F5F5F7]" :
                    isFrozen   ? "border-blue-300/50 bg-blue-50/40" :
                    isExpired  ? "border-[#FF3B30]/30 bg-red-50/50" :
                    ...
                    {freezes.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-black/5 flex flex-col gap-1.5">
                            <p className="text-xs font-medium text-[#86868B]">Freezes</p>
                            {freezes.map(f => (
                                <div key={f.id} className="flex items-center gap-2 text-xs text-[#1D1D1F]">
                                    <span className="flex-1">
                                        {fmtDate(f.freezeStartDate)} · {f.freezeDurationDays} days
                                        {f.notes && <span className="text-[#86868B]"> — {f.notes}</span>}
                                    </span>
                                    <button
                                        onClick={() => handleDeleteFreeze(f)}
                                        className="text-[#FF3B30] hover:text-red-700 transition-colors cursor-pointer"
                                    >
                                        Remove
                                    </button>
                                </div>
                                ...
                                <tr key={tx.id} className={`border-b border-[#F0F0F5] ${i % 2 === 0 ? "bg-white" : "bg-[#F5F5F7]/40"}`}>
                                    ...
                                    <td className="px-4 py-2.5 whitespace-nowrap">
                                        {tx.startMode === "custom" && tx.subscriptionStartDate
                                            ? <span className="text-[#1D1D1F]">{fmtDate(tx.subscriptionStartDate)}</span>
                                            : tx.startMode === "queued"
                                    ...
                                    <td className="px-4 py-2.5 text-[#86868B] capitalize">{tx.type}</td>
                                    ...
                                    <td className="px-4 py-2.5 text-right whitespace-nowrap">
                                        <div className="flex items-center gap-2 justify-end">
                                            <button
                                                onClick={() => openEdit(tx)}
                                                className="text-xs text-[#007AFF] hover:text-[#0056CC] transition-colors cursor-pointer"
                                            >
                                                Edit
                                            </button>
                                            {tx.status === "completed" && (
                                            ...
                                            <button
                                                ...
                                                className="text-xs text-[#FF3B30] hover:text-red-700 transition-colors cursor-pointer"
                                            >
                                                Delete
                                            </button>
                                        </div>
                ...
                <form onSubmit={handleAdd} className="flex flex-col gap-4">
                    <div>
                        ...
                        {addPkg && (
                            <div className="flex gap-4 mt-2 text-xs text-[#86868B]">
                                <span>Duration: <span className="text-[#1D1D1F] font-medium">{addPkg.duration} days</span></span>
                                <span>Price: <span className="text-[#1D1D1F] font-medium">{addPkg.price.toLocaleString()} {addPkg.currency}</span></span>
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        <label className="block text-sm text-[#86868B] mb-1">
                            Subscription Start Date <span className="text-[#86868B]/60">(empty = on first plan activation)</span>
                        </label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label className="block text-sm text-[#86868B] mb-1">Notes</label>
                        <textarea
                            rows={2}
                            value={addNotes}
                    ...
                    <div>
                        <label className="block text-sm text-[#86868B] mb-1">Proof of Payment</label>
                        <input
                            type="file"
                            accept="image/*,.pdf"
                        ...
                        />
                        {addProofFile && <p className="text-xs text-[#86868B] mt-1">{addProofFile.name}</p>}
                    </div>

                    {addError && <p className="text-[#FF3B30] text-sm">{addError}</p>}

                    <div className="flex gap-2">
                        <button
                            type="submit"
                            disabled={addSaving}
                            className="flex-1 bg-[#007AFF] hover:bg-[#0056CC] disabled:opacity-50 text-white font-medium px-4 py-2 rounded-xl transition-colors cursor-pointer text-sm"
                        >
                        ...
                        <button
                            ...
                            className="flex-1 bg-[#F0F0F5] hover:bg-[#D2D2D7] text-[#1D1D1F] font-medium px-4 py-2 rounded-xl transition-colors cursor-pointer text-sm"
                        >
                            Cancel
                        </button>
                    </div>
            ...
            <Modal open={showEditModal} onClose={closeEdit} title="Edit Transaction">
                <form onSubmit={handleEdit} className="flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        {editPkg && (
                            <div className="flex gap-4 mt-2 text-xs text-[#86868B]">
                                <span>Duration: <span className="text-[#1D1D1F] font-medium">{editPkg.duration} days</span></span>
                                <span>Price: <span className="text-[#1D1D1F] font-medium">{editPkg.price.toLocaleString()} {editPkg.currency}</span></span>
                            </div>
                        )}
                        {!editPkg && editPkgKey && (
                    ...
                    <div>
                        <label className="block text-sm text-[#86868B] mb-1">
                            Subscription Start Date <span className="text-[#86868B]/60">(empty = queued after previous subscription)</span>
                        </label>
                        <input
                            type="date"
                    ...
                    {/* Proof image */}
                    <div>
                        <label className="block text-sm text-[#86868B] mb-1">Proof of Transaction</label>
                        {editProofUrl && !editProofFile && (
                            <div className="flex items-center gap-2 mb-2">
                                <a href={`${process.env.NEXT_PUBLIC_API_URL}${editProofUrl}`} target="_blank" rel="noreferrer" className="text-xs text-[#007AFF] hover:underline">
                                    View current proof
                                </a>
                                <button type="button" onClick={() => setEditProofUrl(null)} className="text-xs text-[#FF3B30] hover:underline cursor-pointer">
                                    Remove
                                </button>
                            </div>
                        )}
                        <input
                            ...
                            className="w-full text-sm text-[#86868B] file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-[#F0F0F5] file:text-[#1D1D1F] hover:file:bg-[#D2D2D7] cursor-pointer"
                        />
                        {editProofFile && <p className="text-xs text-[#86868B] mt-1">{editProofFile.name}</p>}
                    </div>

                    {editError && <p className="text-[#FF3B30] text-sm">{editError}</p>}
            ...
            <Modal open={showFreezeModal} onClose={() => setShowFreezeModal(false)} title="Add Subscription Freeze">
                <form onSubmit={handleAddFreeze} className="flex flex-col gap-3">
                    ...
                    <div>
                        <label className="block text-sm text-[#86868B] mb-1">Freeze Start Date *</label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label className="block text-sm text-[#86868B] mb-1">Freeze Duration (days) *</label>
                        <input
                            type="number"
                            min="1"
                            placeholder="e.g. 14"
