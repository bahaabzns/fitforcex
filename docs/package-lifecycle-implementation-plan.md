...
6. **(New, from Phase 3b implementation)** The bulk `saveDraft` ("Save All Drafts") endpoint does not currently accept `cycleDays`/`checkInForms`/`durationChoice` — a coach who edits an active plan via "Save All Drafts" (rather than the single-plan "Save Plan" button) will not be prompted to continue/restart, and that path always implicitly "extends" (via Phase 3a's preservation fix) with whatever `cycle_days` the plan already had. Is this acceptable for MVP, or does "Save All Drafts" need the same prompt wired in? Given it saves *multiple* plans in one call, only one of which is typically the active one, the UX for surfacing a single restart/extend prompt there is unclear and was deliberately left unresolved rather than guessed at.
...
- **Phase 3b follow-through on the Phase 3a discovery:** rather than leave the three activation code paths as three independently-patched bespoke `UPDATE`s (Phase 3a's minimal fix), Phase 3b consolidated `savePlanDraft`'s inline `activatePlanInTransaction` callback onto the now cycle-aware shared `activateSinglePlan()` in both modules, generalizing that function to accept a `Pool | PoolClient` so it can run inside `saveSinglePlanDraft`'s existing transaction. The bulk `saveDraft` ("Save All Drafts") path was deliberately left as its own inline `UPDATE` rather than folded onto the shared function — its loop activates one plan among many freshly-inserted rows in a shape the shared function doesn't fit without a larger refactor; it still gets the Phase 3a preservation fix, just not cycleDays/restart-extend wiring (see §18.1 open question below).
- **New discovered nuance — check-in schedule matching survives plan-id churn by client, not by plan id:** the single-plan save path deletes and reinserts the plan row on every save (a pre-existing, deliberate pattern for the full plan tree — see §3.6), so a `check_in_schedules` row's `source_plan_id` (stamped once, at the plan's first activation) no longer matches the plan's *current* row id by the time a later "Restart" edit runs. Restart-triggered schedule resets are therefore matched by `(client_id, source_plan_type)` instead — safe in practice because a client has at most one active plan per type at a time — rather than `source_plan_id`. `source_plan_id`/`source_plan_type` remain purely informational provenance fields, exactly as §13's edge-case table already specified for the *deleted-variation* case; this extends the same reasoning to the *plan itself churning ids on every save* case, which the original edge-case entry didn't call out explicitly.
- **Newly discovered and fixed — activation response wasn't reaching the UI:** `useNutritionPlan.js`'s and `useTrainingPlan.js`'s `handleActivatePlan` only performed an optimistic `status` update before the API call and did nothing with the response afterward, so a successful activation's `activated_at`/`cycle_days`/`cycle_end_at` never reached local state — the new builder header stat row (§11.3/§11.4) silently failed to appear until the next full page load. Reproduced live: activated a plan, the "Days left" stat did not render; only appeared after reselecting the plan (which re-fetches from the server). Fixed by merging the activation response into `plans`/`selectedPlan` state directly (nutrition) and by reusing the hook's own existing refetch-after-write pattern (training, via `fetchClientPlans`) rather than inventing a second merge strategy in the same file. The identical gap existed in `handleSaveSelectedPlan`'s restart/extend response handling (nutrition) and was fixed the same way.
