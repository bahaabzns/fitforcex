...
## Database changes
None beyond Phase 1's schema (already in place). No new migration in this phase — this is a pure application-code phase, which is exactly why it can follow Phase 1 immediately without its own maintenance window.
...
| `form_questions → form_responses` (`question_id`) | `CASCADE` | **Relationship removed entirely** — `form_responses.question_id` now points at `form_version_questions.id` (no cascade needed, since sealed versions/snapshots are never deleted while referenced, by construction) |
...
### Migration scripts
- **`037_form_responses_repoint.js`** — `ALTER TABLE form_responses` drop the FK to `form_questions(id)`, add FK to `form_version_questions(id)` (`question_id` column is reused, values already remapped in Phase 1's backfill).
- **`038_cascade_hardening.js`** — drop and recreate the three `CASCADE` FKs listed above as `RESTRICT` (`forms→form_requests`, `forms→check_in_schedules`, `forms→package_default_forms`).
- **`039_drop_form_questions.js`** — after a bake period (see Release Strategy) and a final backup, `DROP TABLE form_questions`. This migration ships in its own release, deliberately separated from `037`/`038` by at least the agreed bake window, so it can be held back independently if anything unexpected surfaces.
...
- After `037`: `SELECT count(*) FROM form_responses WHERE question_id NOT IN (SELECT id FROM form_version_questions)` → must be `0`.
...
### Rollback strategy
- `037`/`038` rollback: recreate the dropped FKs pointing back at their prior targets/cascade behavior. Safe as long as `037`/`038` haven't been followed by `039` yet — recreating a cascade FK is non-destructive by itself.
